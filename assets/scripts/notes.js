...
class NotesApp {
    ...
    showToast(message, duration = 3000) {
        ...
        let icon = 'ℹ️';
        if (m.includes('clipboard') || m.includes('copied') || m.includes('link copied')) icon = '📋';
        else if (m.includes('reconnected')) icon = '✅';
        else if (m.includes('reconnect')) icon = '🔄';
        else if (m.includes('joined shared') || m.includes('joined collab')) icon = '🤝';
        else if (m.includes('session closed') || m.includes('ended this session')) icon = '🚪';
        else if (m.includes('view only')) icon = '👁️';
        else if (m.includes('can now edit') || m.includes('permission changed')) icon = '✏️';
        else if (m.includes('failed') || m.includes('error')) icon = '❌';
        else if (m.includes('limit') || m.includes('only have 1') || m.includes('already have') || m.includes('large') || m.includes('select an image') || m.includes('no text')) icon = '⚠️';
        else if (m.includes('image')) icon = '🖼️';
